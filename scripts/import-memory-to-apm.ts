/**
 * Read memory/persistence/index.md and write bounded summaries into `.apm` sections.
 *
 * Writes `persist` and `dynamicDetail` only. Does not create chunks or todos;
 * curate long-form knowledge manually into persist and track execution in dynamic detail.
 */
...
async function main() {
  ...
  let persistCore =
    ...
    `本脚本不再写入 chunks 或 todos；请将稳定规则与结论整理进 persist，阶段进展写在 apm dynamic detail。`;
  ...
  let detailText = [
    ...
    `下一步：逐条打开 records，提炼后写入 persist；当前执行细节用 apm dynamic detail 跟踪。`,
    `说明：全文不在此脚本落盘；无自动 chunks/todos 归档。`
  ].join("\n");
  if (countChars(detailText) > 1000) detailText = clipToMaxCountChars(detailText, 1000);
  const detailFinal = padToMinCountChars(detailText, 500, "（dynamic detail 长度在配置 min~max 内。）");
